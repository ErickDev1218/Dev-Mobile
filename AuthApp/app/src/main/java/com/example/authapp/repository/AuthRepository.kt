...
class AuthRepository(
    ...
    suspend fun register (email : String, password : String) : Result<UserOut> {
        ...
        return if (response.isSuccessful) Result.success(response.body()!!) else Result.failure(Exception(response.errorBody()?.string()))
    }
    ...
    suspend fun login (email : String, password: String) : Result<String>{
        ...
        return if (response.isSuccessful) {
            ...
            Result.failure(Exception("Usuário ou senha inválida"))
        }
    }
